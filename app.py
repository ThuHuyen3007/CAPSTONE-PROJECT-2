...
def main():
    ...
    if request.method == 'POST':
      ...
      'Location Code':loc,'EmploymentStatus':emp,'Months Since Last Claim':cla,'State':sta}
        ...
        if gr==0:
            reg_model = pickle.load(open('xgb0.pkl', 'rb'))
        elif gr==1:
            reg_model = pickle.load(open('xgb1.pkl', 'rb'))
        elif gr==2:
            reg_model = pickle.load(open('xgb2.pkl', 'rb')) 
        elif gr==3:  
            reg_model = pickle.load(open('xgb3.pkl', 'rb'))
        elif gr==4:
            reg_model = pickle.load(open('xgb4.pkl', 'rb'))
        elif gr==5:   
            reg_model = pickle.load(open('xgb5.pkl', 'rb'))
